...
CRC = [1000/20+SR*200/4, 0.02+0.16]; % [install cost ($/kW/year), maintenance cost + operational cost ($/kWh)]
...
%                          SOLVER                                %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[cost_houston,capacity_houston] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),RR,RC,SR,CRR,CRC,P,GP,RP,BP,1,'results\houston_bio');
[cost_houston_noPV,capacity_houston_noPV] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),[0,0],RC,SR,CRR,CRC,P,GP,RP,BP,1,'results\houston_noPV_bio');
[cost_houston_givenPV,capacity_houston_givenPV] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),[250,250],RC,SR,CRR,CRC,P,GP,RP,BP,1,'results\houston_givenPV_bio');
[cost_houston_givenGE,capacity_houston_givenGE] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),RR,RC,SR,[1400,1400],CRC,P,GP,RP,BP,1,'results\houston_givenGE_bio');
[cost_houston_givenBoth,capacity_houston_givenBoth] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),[250,250],RC,SR,[1400,1400],CRC,P,GP,RP,BP,1,'results\houston_givenBoth_bio');
...
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\expenditure_comp_houston_bio.eps');
saveas(gcf,'results\expenditure_comp_houston_bio.fig');
...
set(gca,'xticklabel',{'Our solution','no PV','PV = 250kW','GE=1.4mW','PV = 250kW,GE=1.4mW'});
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\capacity_comp_houston_bio.eps');
saveas(gcf,'results\capacity_comp_houston_bio.fig');

save('results\houston_bio.mat');
