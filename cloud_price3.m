...
[A,B,S,E] = batch_job_generator(T,N,'Uniform',11.99,11.99,'Uniform',1,1,C*BM);
A = A';
B = B';
NB = PV/MR;
...
if scheduling == 1 || scheduling == 0
    ...
    print ('-depsc', 'results\cloud3-opt.eps');
    saveas(gcf,'results\cloud3-opt.fig')
end
...
if scheduling == 3 || scheduling == 0
    ...
    set(LEG,'FontSize',8)
    set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.1]);
    print ('-depsc', 'results\cloud3-be.eps');
    saveas(gcf,'results\cloud3-be.fig')
end
...
saveas(gcf,'results\cloud3-comp.fig')
%{
if scheduling == 0
