...
IP = 0.2; % idle power consumption of a server, in kW
...
plot_option = 1;
for i = 1:1:6
    con(i) = 0.4+0.1*i; % maximum utilization after consolidation
    [cost_houston(i,:),capacity_houston(i,:),G,status] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con(i),BS,A,S,E,PUE,R(1,:),RR,RC,SR,CRR,CRC,P,GP,RP,BP,plot_option,'results\houston_u');
end
...
figure;
plot(con,sum(cost_houston,2));
xlabel('utilization');
xlim([0.5,1]);
ylabel('Annual expenditure ($)');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\cost_u.eps');
saveas(gcf,'results\cost_u.fig');

figure;
bar(con,cost_houston,'stacked');
xlabel('utilization');
xlim([0.4,1.1]);
ylabel('Annual expenditure ($)');
legend('PV infrastructure cost','PV O&M cost','GE infrastructure cost','GE O&M cost','Location','NorthEast');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\cost_u.eps');
saveas(gcf,'results\cost_u.fig');
...
figure;
plot(con,capacity_houston(:,1),'r-',con,capacity_houston(:,2),'b-');
xlabel('utilization');
xlim([0.5,1]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\capacity_u.eps');
saveas(gcf,'results\capacity_u.fig');

figure;
bar(con,capacity_houston,'stacked')
xlabel('utilization');
xlim([0.4,1.1]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');

save('results\houston_u.mat');
