function [cost,capacity,emission,ratio] = houston_grid_BiMarkets(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
...
cvx_begin
    ...
    minimize (24*365/T*sum(RC(2)*PV*solar' + CRC(2:T+1)'.*G + GP'.*pos(D-DC_power*bi_power_ratio) )...
                + RC(1)*PV + CRC(1)*GE ...
             )
    subject to
        PV >= range1(1);
        ...
        %D-C*bi_power_ratio >=0; % Bilateral markets constraint        
cvx_end
...
cost = [PV * RC(1), 24*365/T*sum(RC(2)*PV*solar'), GE * CRC(1), 24*365/T*sum(CRC(2:T+1)'.*G),24*365/T*sum(GP'.*abs(D-DC_power*bi_power_ratio) + DC_power*bi_power_ratio*bi_rate)];
...
%}
FontSize = 12;
wholesale = DC_power*bi_power_ratio*ones(1,T);
grid_power = max(wholesale,D');
if plt >= 1
    figure;
    data_range = [1:24];
    bar(data_range,[grid_power(data_range);PV*solar(data_range);G(data_range)']','stacked');
    hold on;
    ...
    plot(data_range_2-0.5,wholesale(data_range_2),'-.m','LineWidth', 2);
    ...
    legend('Grid power','PV generation','GE generation','Power demand','Wholesale purchase', 'FontSize', FontSize,'Location', 'northwest');
    ylim([0,C/OP*1.1]);
