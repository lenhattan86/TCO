function [cost,capacity,emission,ratio, D] = houston_grid_tou(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
    con,BS,A,S,E,bu,PUE,solar,range1,RC,RE,SR,range2,CRC,CRE,P,GP,RP,BP,plt,location)
...
if plt >= 1
    ...
    shift = 0*24;
    day_range = 1:24;
    data_range = shift+day_range;
    
    night_hours = [1 2 3 4 5 6 22 23 24];
%     bar(night_hours,ones(1,length(night_hours))*99999999,1,'FaceColor',[95 130 105]/255,'edgecolor','none');
    bar(night_hours,ones(1,length(night_hours))*99999999,1,'FaceColor',[180 250 160]/255,'edgecolor','none');
    hold on;
    peak_hours = [13 14 15 16 17 18];
    bar(peak_hours,ones(1,length(peak_hours))*99999999,1,'FaceColor',[160 250 250]/255,'edgecolor','none');
    hold on;
    off_peak_hours = [7 8 9 10 11 12 19 20 21];
    bar(off_peak_hours,ones(1,length(off_peak_hours))*99999999,1,'FaceColor',[250 160 250]/255,'edgecolor','none');
    hold on;    
    
    bar(day_range,[D(data_range)';PV*solar(data_range);G(data_range)']','stacked');
    hold on;    
    ...
    plot(day_range,power_demand(data_range),'r', 'LineWidth', 2);
...
%     legend('Night hours', 'peak hours', 'off-peak hours', 'Grid power', 'PV generation','GE generation','Power demand', 'FontSize', FontSize,'Location', 'northeast');    
    ylim([0,C/OP*1.1]);
