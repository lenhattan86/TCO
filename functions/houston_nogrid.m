function [cost,capacity,emission] = houston_nogrid(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au,con,BS,A,S,E,bu,PUE,solar,range1,RC,RE,SR,range2,CRC,CRE,P,GP,RP,BP,plt,location)
...
cvx_begin
    ...
    minimize (24*365/T*sum(RC(2)*PV*solar' + CRC(2)*G + GP'.*D)  + RC(1)*PV + CRC(1)*GE)
    subject to
        PV >= range1(1);
        PV <= range1(2);
...
cost = [PV * RC(1), 24*365/T*sum(RC(2)*PV*solar'), GE * CRC(1), 24*365/T*sum(CRC(2)*G),24*365/T*sum(GP*D)];
...
if plt >= 1
    ...
    hold on;
    plot(1:T,(idle_power+sum(a_houston_power,1)+sum(b1/PP*(PP-IP),1)+sum(b2/PP*(PP-IP),1)).*PUE,'r',1:T,C/OP*ones(1,T),'k', 'LineWidth', 2)
    xlabel('hour');
    ylabel('kW');
    ylim([0,C*1.5]);
