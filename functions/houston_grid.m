function [cost,capacity,emission,ratio, D] = houston_grid(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
...
if plt >= 1
    ...
    plot(1/24:1/24:T/24,(idle_power+sum(a_houston_power,1)+sum(b1/PP*(PP-IP),1)+sum(b2/PP*(PP-IP),1)).*PUE,'r', 'LineWidth', 2)
    xlabel('month');
    ylabel('power (kW)');
