function [cost,capacity,emission,ratio] = houston_grid_SR(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
    con,BS,A,S,E,bu,PUE,solar,range1,RC,RE,SR,range2,CRC,CRE,P,GP,RP,BP ,...
    sr_rates, ...
    plt,location)

[temp1,temp2,temp3,temp4, D_real] = houston_grid(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
    con,BS,A,S,E,bu,PUE,solar,range1,RC,RE,SR,range2,CRC,CRE,P,GP,RP,BP,0,'temp');
...
%      minimize (24*365/T*sum(RC(2)*PV*solar' + CRC(2:T+1)'.*G + (GP'-sr_rates').*(D))  ...                    
%                     + RC(1)*PV + CRC(1)*GE ...
%                  )
    subject to
        PV >= range1(1);
        ...
        SR <= D_real; % Spinning reserve constraint        
        SR >= 0;
        sr_rates'.*(D + SR) == sr_rates'.*D_real;
cvx_end
...
cost = [PV * RC(1), 24*365/T*sum(RC(2)*PV*solar'), GE * CRC(1), 24*365/T*sum(CRC(2:T+1)'.*G),24*365/T*sum(GP'.*D - (sr_rates'.*SR))];
...
%     legend('SR hour','Grid power','PV generation','GE generation','Power demand', 'FontSize', FontSize,'Location', 'northwest');
    ylim([0,C/OP*1.1]);
    xlim([0.5,length(data_range)]);
