function [cost,capacity,emission,ratio] = houston_grid_IBR(colocate,nz,ds,off,T,IP,PP,OP,C,CO2_grid,a,au, ...
    con,BS,A,S,E,bu,PUE,solar,range1,RC,RE,SR,range2,CRC,CRE,P,GP,RP,BP ,...
    ibr_levels, ibr_rates ,...
    plt,location)
...
cvx_begin
    ...
    subject to
        ...
        %IBR = ibr_rates'*pos(ones(ibr_len,1)*D-IBR_LEVELS); % Inclining Block Rates
cvx_end
...
cost = [PV * RC(1), 24*365/T*sum(RC(2)*PV*solar'), GE * CRC(1), 24*365/T*sum(CRC(2:T+1)'.*G), 24*365/T*sum(GP'.*D + (ibr_rates*pos(ones(ibr_len,1)*D'-IBR_LEVELS))')];
...
if plt >= 1
    ...
    hold on;
    ibr_level_1 = IBR_LEVELS(1,:);
    data_range_2 = [1:25];
    plot(data_range_2-0.5,ibr_level_1(data_range_2),'--g','LineWidth', 2);
    hold on;
    ibr_level_2 = IBR_LEVELS(2,:);
    plot(data_range_2-0.5,ibr_level_2(data_range_2),'-.m','LineWidth', 2);
...
%     legend('PV generation','GE generation','Grid power','Power demand', 'FontSize', FontSize,'Location', 'northwest');
    legend('Grid power','PV generation','GE generation','Power demand','IBR level 1', 'IBR level 2', 'FontSize', FontSize,'Location', 'northwest');
    ylim([0,C/OP*1.1]);
