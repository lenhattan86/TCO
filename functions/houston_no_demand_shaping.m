function [cost,capacity,G] = houston_no_demand_shaping(T,IP,PP,OP,C,CO2_grid,a,au,con,BS,A,S,E,PUE,solar,range1,RC,SR,range2,CRC,P,GP,RP,BP,plt,location)
a_houston_server = ceil(a./(au'*ones(1,T))/PP);
a_houston_power = a_houston_server*(IP + (PP-IP)*au);
a_remaining = a_houston_server*PP*(con-au);
b_flat = BS./sum(A,2)*ones(1,T);
cvx_begin
