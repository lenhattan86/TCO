...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure;
plot(ratio,sum(cost,2)./demand');
xlabel('Flexible workload ratio');
xlim([0.05,1]);
ylabel('Annual expenditure per job');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\average_cost_ratio.eps');
saveas(gcf,'results\average_cost_ratio.fig');

figure;
bar(ratio,cost,'stacked');
xlabel('Flexible workload ratio');
xlim([0,1.05]);
...
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\cost_ratio.eps');
saveas(gcf,'results\cost_ratio.fig');



figure;
plot(ratio,capacity(:,1),'r-',ratio,capacity(:,2),'b-');
xlabel('Flexible workload ratio');
xlim([0.05,1]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\capacity_ratio.eps');
saveas(gcf,'results\capacity_ratio.fig');

figure;
bar(ratio,capacity,'stacked')
xlabel('Flexible workload ratio');
xlim([0,1.05]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');

save('results\houston_ratio.mat');
%{
figure;
