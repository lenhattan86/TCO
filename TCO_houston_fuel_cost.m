...
for i = 1:1:16
    fuel(i) = 0.01*i;
    CRC(2) = 0.02+fuel(i);
    [cost(i,:),capacity(i,:)] = houston(T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,PUE,R(1,:),[250,250],RC,SR,CRR,CRC,P,GP,RP,BP,0,'results\tmp')
end
...
print ('-depsc', 'results\cost_fuel_cost_250.eps');
saveas(gcf,'results\cost_fuel_cost_250.fig');


figure;
plot(fuel,capacity(:,1),'r-',fuel,capacity(:,2),'b-');
xlabel('gas cost ($/kWh)');
xlim([0.01,0.16]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results\capacity_fuel_cost_250.eps');
saveas(gcf,'results\capacity_fuel_cost_250.fig');

figure;
bar(fuel,capacity,'stacked')
xlabel('gas cost ($/kWh)');
xlim([0,0.17]);
ylabel('capacity (kW)');
legend('PV','GE','Location','NorthWest');

save('results\houston_fuel_cost_250.mat');
%{
figure;
