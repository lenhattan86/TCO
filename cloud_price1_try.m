...
virtual_machine_infrastructure_cost = 1; % in cents
number_virtual_machine = 1000; 
C = number_virtual_machine;
power_consumption_per_virtual_machine = 0.2; % in kW
PUE = 1.3;
T = 24;
batch_ratio = 0.8;
...
step_size = 0.05;
batch_size = 1;
batch_arrival = batch_size*ceil(random('Uniform',1,20,(bid_high-bid_low)/step_size,T));
batch_arrival = batch_arrival/sum(sum(batch_arrival))*T*C*batch_ratio;
% PV generation
...
ep = 10*power_consumption_per_virtual_machine*ones(1,T);
oc_original = virtual_machine_infrastructure_cost*C;
...
figure;
stairs(sum(batch_finish,1)');
hold on;
plot(1:T,C*ones(1,T),'k',1:T,R,'r', 'LineWidth', 2);
xlim([1,T]);
xlabel('hour');
ylabel('number of virtual machine');
legend('total virtual machine','IT capacity','renewable')
...
csvwrite('results\price1_summary_new.csv',[total_revenue,total_cost,total_oc,total_profit/T/C,total_demand/T/C,avg_price;...
    total_revenue_flat,total_cost_flat,total_oc_flat,total_profit_flat/T/C,total_demand_flat/T/C,avg_price_flat;...
    total_revenue_low,total_cost_low,total_oc_low,total_profit_low/T/C,total_demand_low/T/C,avg_price_low;...
    total_revenue_high,total_cost_high,total_oc_high,total_profit_high/T/C,total_demand_high/T/C,avg_price_high;...
    total_revenue_binary,total_cost_binary,total_oc_binary,total_profit_binary/T/C,total_demand_binary/T/C,avg_price_binary;...
    total_revenue_mu,total_cost_mu,total_oc_mu,total_profit_mu/T/C,total_demand_mu/T/C,avg_price_mu]);
