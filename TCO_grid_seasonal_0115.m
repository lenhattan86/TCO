...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load_exist = 0;
mat_location = 'results/houston_payback_seasonal_1217.mat';

if load_exist == 1
    load('results/houston_payback_seasonal_1217.mat');
else
% general parameters
...
R = R/max(R);
RR = [0, DC_power]; % renewable capacity range, in kW
...
CRR = [0,DC_power]; % controled renewable capacity range, in kW
...
CRC = [1000/20+SR*200/4, 0.005+0.085*ones(1,T)];
...
GP = 0.12*ones(1,T); % grid power only as backup
...
[cost_demand,capacity_demand,emission_demand] = houston_grid(1,1,1,1,T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,bu,PUE,R(1,:),0.5*[DC_power,DC_power],RC,RE,SR,[DC_power,DC_power],CRC,CRE,P,GP,RP,BP,plot_option,'results/nz-demand');
...
% net-zero, integrated
[cost_int,capacity_int,emission_int] = houston_grid(1,1,1,1,T,IP,PP,OP,IT,CO2_grid,a,au,con,BS,A,S,E,bu,PUE,R(1,:),RR,RC,RE,SR,CRR,CRC,CRE,P,GP,RP,BP,plot_option,'results/nz-int');
...
set(gca,'xticklabel',{'Grid only','Supply opt','Supply given','Our solution'});
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 5.0 2.8]);
print ('-depsc', 'results/emission_comp.eps');
eps2pdf('results/emission_comp.eps','/usr/local/bin/gs');
